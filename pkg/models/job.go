...
//total, _ := sess.Join("INNER", "package_pkg", "package_pkg.pid = package_job.pid").Where("package_pkg.title=?",filter.Title).And("package_job.uid = ?", filter.Owner)..Count(&jobs)
if p != nil {
		n, _ := sess.Join("INNER", "package_pkg", "package_pkg.pid = package_job.pid").Where("package_pkg.title=?",filter.Title).And("package_job.uid = ?", filter.Owner)..Count(&jobs)
		p.TotalNum = n
		sess = sess.Limit(p.PageSize, (p.PageNum-1)*p.PageSize)// it works
	}
...
if err := sess.Join("INNER", "package_pkg", "package_pkg.pid = package_job.pid").Where("package_pkg.title=?",filter.Title).And("package_job.uid = ?", filter.Owner).Find(&jobs); err != nil {
 ...
 return jobs, nil
	...
	if filter.Title != "" || (filter.Owner != "" && typ == cydex.UPLOAD){
if p != nil {
		n, _ := sess.Join("INNER", "package_pkg", "package_pkg.pid = package_job.pid").Where("package_pkg.title=?",filter.Title).Or("package_job.uid = ?", filter.Owner).Count(&jobs)
		p.TotalNum = n
		sess = sess.Limit(p.PageSize, (p.PageNum-1)*p.PageSize)// it works
	}
